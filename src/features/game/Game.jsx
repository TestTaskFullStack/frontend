...
import { useStartGameMutation, useListenUserStartedGameEventQuery } from "../../services/socket"
...
const Game = () => {
    ...
    useCallToaster(eventComment, refetch)
    ...
    return (
        <Box display="flex" maxW="1200px" gap="20px" alignItems="start" flexDirection="row" p={4} width="100vw" justifyContent="space-between" h="80px">
            <Box>
                ...
                <Box display="flex" gap="10px" mb="10px">
                    {game.data && game.data.inTop && <GameInfoBadge title="Популярна" colorPalette="blue" icon={true} />}
                    {game.data.genre && <GameInfoBadge title={game.data.genre.name} colorPalette="green" icon={false} />}
                </Box>
                <ButtonGreen w="300px" handlePlay={handleStartGame}>Запустити гру</ButtonGreen>
                <GameCommentForm />
                <GameCommentsList comments={game.data.comments} />
            </Box>
            <Image rounded="10px" w="500px" h="500px" src={game.data.gameBoxArt} alt={game.data.commonGameName} />
        </Box>
    )
}
